...
TEST(ScalarMixedOperationsTest, Test2) {
    ...
    EXPECT_FLOAT_EQ(s4->grad, 0.1);
    EXPECT_FLOAT_EQ(s5->grad, -0.05);
}
...
TEST(ScalarMixedOperationsTest, Test3) {
    ...
    EXPECT_FLOAT_EQ(s2->grad, 2.0);
    EXPECT_FLOAT_EQ(s3->grad, 3.0);
    EXPECT_FLOAT_EQ(s4->grad, -0.5);
    EXPECT_FLOAT_EQ(s5->grad, 0.25);
}
...
TEST(ScalarExp, Test2) {
    auto s1 = std::make_shared<Scalar<float>>(0.0);
    auto s2 = exp(s1);
...
TEST(ScalarExp, Test4) {
    auto s1 = std::make_shared<Scalar<float>>(-2.0);
    auto s2 = exp(s1);
...
TEST(ScalarLog, Test3) {
    auto s1 = std::make_shared<Scalar<float>>(10.0);
    auto s2 = log(s1);
